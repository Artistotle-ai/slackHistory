...
describe('slack-utils', () => {
  ...
  describe('getMessageChannelId', () => {
    ...
    it('should prefer channel over channel_id even if channel_id is provided first', () => {
      const event = { channel_id: 'C222222', channel: 'C111111' };
      const result = getMessageChannelId(event);
      expect(result).toBe('C111111');
    });
  });

  describe('getChannelEventChannelId', () => {
    it('should return channel ID from string channel', () => {
      const event = { type: 'channel_archive', channel: 'C123456' };
      const result = getChannelEventChannelId(event);
      expect(result).toBe('C123456');
    });

    it('should return channel ID from channel object', () => {
      const event = { type: 'channel_created' as const, channel: { id: 'C123456', name: 'test-channel' } };
      const result = getChannelEventChannelId(event);
      expect(result).toBe('C123456');
    });

    it('should prefer string channel over channel object', () => {
      const event = { 
        type: 'channel_rename', 
        channel: 'C111111' as any // TypeScript expects object for channel_rename, but function handles string
      };
      const result = getChannelEventChannelId(event);
      expect(result).toBe('C111111');
    });

    it('should handle channel_created event with channel object', () => {
      const event = { 
        type: 'channel_created' as const, 
        channel: { id: 'C789', name: 'new-channel' } 
      };
      const result = getChannelEventChannelId(event);
      expect(result).toBe('C789');
    });

    it('should handle channel_rename event with channel object', () => {
      const event = { 
        type: 'channel_rename' as const, 
        channel: { id: 'C456', name: 'renamed-channel' } 
      };
      const result = getChannelEventChannelId(event);
      expect(result).toBe('C456');
    });
  });
});
